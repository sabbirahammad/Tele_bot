...
from database import add_user, get_all_channels, verify_user, get_user_status, DB_PATH, get_channel_invite_link, user_exists, is_user_verified, add_referral, get_user_balances
...
async def start_handler(bot, message):    # অ্যাড দেখে ফিরে আসা ইউজারদের হ্যান্ডেল করা
    try:
            ...
            if param == "verify_daily":
                ...
                if not user_exists(user_id):
                    if not message.from_user.username:
                        await message.reply_text("❌ দুঃখিত, আপনার অ্যাকাউন্টে ইউজারনেম নেই। ফেক রেফারেল রোধে ইউজারনেম থাকা বাধ্যতামূলক।")
                        return
                    
                    # হিউম্যান ভেরিফিকেশন বাটন পাঠানো
                    verification_buttons = [[
                        InlineKeyboardButton("✅ আমি মানুষ (Verify)", callback_data=f"v_ref_{inviter_id}")
                    ]]
                    await message.reply_text(
                        "🛡️ **হিউম্যান ভেরিফিকেশন**\n\nরেফারেল বোনাসটি নিশ্চিত করতে নিচের বাটনে ক্লিক করুন। এটি শুধুমাত্র একবার করতে হয়।",
                        reply_markup=InlineKeyboardMarkup(verification_buttons)
                    )
                    return

            elif param.startswith("file_"):
                # সরাসরি ফাইল লিংকেও ভেরিফিকেশন চেক যোগ করা হলো
                if not is_user_verified(message.from_user.id):
                    from plugins.search import send_daily_verification_message
                    return await send_daily_verification_message(bot, message, param)
        ...
        coin_bal = balances[0] if balances else 0.0
        fly_bal = balances[1] if balances else 0.0
